...
-   `igraph::is_weighted()` only returns true if there is a "weight" column
    -   [stackoverflow](https://stackoverflow.com/questions/51856706/weighted-graph-from-a-data-frame) \> In igraph edge weights are represented via an edge attribute, called 'weight'. The is_weighted function only checks that such an attribute exists. (It does not even checks that it is a numeric edge attribute.)

```{r}
links |>
  dplyr::mutate(weight = "cake") |>
  igraph::graph_from_data_frame(vertices = edges) |>
  igraph::is.weighted()
```
...
-   `create_bipartite()`: too generic/unstructured
...
-   `as_tbl_graph(igraph)` -- just subclasses `igraph` into `tbl_graph`:
